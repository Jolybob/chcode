export class DataManager {
  ...
  async load(){const files=['classes','enemies','spells','dungeons','progression','balance','items','crafting'];const values=await Promise.all(files.map(async name=>[name,await (await fetch(`data/${name}.json?v=10`,{cache:'no-store'})).json()]));this.raw=Object.fromEntries(values);return this.raw}
  ...
  exportBundle(extra={}){return {schemaVersion:3,exportedAt:new Date().toISOString(),config:this.raw,...extra}}
